...
extcontract_template(OurChain, TxList, Ledger, CheckFun) ->
  ...
  try
  Test=fun(LedgerPID) ->
			   %debug_tools:display_address_storage(LedgerPID, <<128,0,32,0,150,0,0,2>>),
...
">>),
      ...
      TxList1=[
               {<<"1deploy">>, maps:put(sigverify,#{valid=>1},TX1)},
               {<<"2call1">>, maps:put(sigverify,#{valid=>1},TX2)},
               {<<"3xfer">>, maps:put(sigverify,#{valid=>1},TX3)},
               {<<"4call2">>, maps:put(sigverify,#{valid=>1},TX4)}
              ],
...
io:format("R ~p~n",[R]),
      ...
      io:format("Sets ~p~n",[L1]),
      [
	   ?assertMatch(#{<<128,0,32,0,150,0,0,2>> := #{amount := #{<<"SK">> := 1}}}, L1),
	   ?assertMatch(#{<<128,0,32,0,150,0,0,1>> := #{amount := #{<<"SK">> := 2}}}, L1)
      ].
...
">>),
	  ...
	  LogExtract = lists:foldl(
		fun([_,<<"4deploy_w_transfer">>,_,Res,Addr|_], A) ->
				A#{tx4_res => Res,
				   tx4_addr => Addr};
		   ([_,<<"1testdeploy">>,_,Res,Addr|_], A) ->
				A#{tx1_res => Res,
				   tx1_addr => Addr};
		   ([_,<<"2call">>,_,Res,Addr|_], A) ->
				A#{tx2_res => Res,
				   tx2_addr => Addr};
      ...
      [
	   ...
	   ?assertMatch(#{code:=<<_/binary>>,
					  state:=#{<<0>>:=_}}, maps:get(ExtractAddr(maps:get(tx2_addr, LogExtract)),L1)),
	   ?assertMatch(1,maps:get(tx3_res, LogExtract)),
	   ?assertMatch(<<>>,maps:get(tx3_addr, LogExtract)),
	   ?assertMatch(1,maps:get(tx4_res, LogExtract)),
	   ?assertMatch(Addr1,maps:get(tx1_addr, LogExtract)),
	   ?assertMatch(#{amount := #{<<"FTT">> := 1,<<"SK">> := 1}},
					maps:get(maps:get(tx4_addr, LogExtract),L1)),
	   ?assertEqual(
...
#{"sc2" => binary:decode_unsigned(SC1)}
      ...
      register(eevm_tracer,self()),
      {ok,L1,_Rec}=extcontract_template(OurChain, TxList1, Ledger, TestFun),
      unregister(eevm_tracer),
      _ContractLedger=maps:get(Addr1,L1),
      CallRet=recv_callret(),
      io:format("CallRet ~p~n",[CallRet]),
      io:format("Receipt ~p~n",[_Rec]),
      [
       ?assertMatch([{<<>>,0},{<<>>,1}],CallRet)
      ].
...
tether_test() ->
      ...
      TX1=tx:sign(
            tx:construct_tx(#{
              ...
              txext=>#{ "code"=> Code, "vm" => "evm", "deploy" => "inplace" }
             }), Pvt1),
      TX2=tx:sign(
            tx:construct_tx(#{
              ver=>2,
       ...
       TX3=tx:sign(
            tx:construct_tx(#{
              ...
              payload=>[
                        #{purpose=>gas, amount=>33000, cur=><<"FTT">>},
                        #{purpose=>srcfee, amount=>2, cur=><<"FTT">>}
                       ],
              seq=>4,
              t=>os:system_time(millisecond)
      ...
      TX4=tx:sign(
            tx:construct_tx(#{
              ...
              payload=>[
                        #{purpose=>gas, amount=>40000, cur=><<"FTT">>},
                        #{purpose=>srcfee, amount=>2, cur=><<"FTT">>}
                       ],
              seq=>5,
              t=>os:system_time(millisecond)
      ...
      TxList1=[
               ...
               {<<"4transfer1">>, maps:put(sigverify,#{valid=>1},TX4)},
               {<<"5transfer2">>, maps:put(sigverify,#{valid=>1},TX5)}
              ],
      TestFun=fun(#{block:=Block,
                    emit:=_Emit,
                    failed:=Failed}) ->
      ...
      Ledger=[
              {Addr2,
               #{amount => #{ <<"FTT">> => 100000, <<"SK">> => 3, <<"TST">> => 26 }}
              },
              {Addr1,
               #{amount => #{ <<"FTT">> => 1000000, <<"SK">> => 3, <<"TST">> => 26 }}
              }
             ],
      {ok,L1,R}=extcontract_template(OurChain, TxList1, Ledger, TestFun),
	  io:format("Receipt ~p~n",[R]),
      {ok,<<B1:256/big>>} = contract_evm:call(Addr1,L1, "balanceOf(address)", [Addr1]),
      {ok,<<B2:256/big>>} = contract_evm:call(Addr1,L1, "balanceOf(address)", [Addr2]),
      {ok,<<B3:256/big>>} = contract_evm:call(Addr1,L1, "balanceOf(address)", [Addr3]),
      {ok,<<B4:256/big>>} = contract_evm:call(Addr1,L1, "balanceOf(address)", [<<12345:256/big>>]),
...
acc_sum(#{amount:=Amounts}, A) ->
	maps:fold(fun(Token,Amount,A1) ->
					  maps:put(Token,Amount+maps:get(Token,A1,0),A1)
			  end, A, Amounts).
