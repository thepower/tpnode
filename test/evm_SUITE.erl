...
evm_sponsor_test(Config) ->
  ...
  {ok, TxId0} = make_transaction(Node,(EAddr), Addr, Seq+1, <<"FTT">>,
                                 1000000000, <<"hello to EVM">>, EPriv, [nowait]),
  ...
  {ok, #{<<"txid">> := _TxID1, <<"res">>:=TxID1Res}} = tpapi2:submit_tx(Node, DeployTx),
  [
  ?assertMatch(<<"ok">>, TxID1Res)
  ].

evm_chainstate_test(Config) ->
  {endless_addr,EAddr}=proplists:lookup(endless_addr,Config),
  {endless_addr_pk,EPriv}=proplists:lookup(endless_addr_pk,Config),
%
  ?assertMatch(true, is_binary(EAddr)),
  ...
  {ok,MyAddr}=tpnode_reporter:ensure_account(),
  logger("New wallet ~p / ~p~n",[Wallet,MyAddr]),
  {ok, MTxId0} = make_transaction(Node,(EAddr), MyAddr, Seq+1, <<"FTT">>,
                                 1000000, <<"hello to EVM">>, EPriv, [nowait]),
  {ok, MTxId1} = make_transaction(Node,(EAddr), MyAddr, Seq+2, <<"SK">>,
                                 200, <<"Few SK 4 U">>, EPriv, [nowait]),
  logger("TxId0: ~p", [MTxId0]),


  logger("New wallet ~p / ~p~n",[Wallet,Addr]),
  {ok, TxId0} = make_transaction(Node,(EAddr), Addr, Seq+3, <<"FTT">>,
                                 1000000, <<"hello to EVM">>, EPriv, [nowait]),
  {ok, TxId1} = make_transaction(Node,(EAddr), Addr, Seq+4, <<"SK">>,
                                 200, <<"Few SK 4 U">>, EPriv, [nowait]),
  logger("TxId0: ~p", [TxId0]),
  ...
  {ok, _TxIDB } = tpapi2:submit_tx(Node, SetCFTx, [nowait]),
  ...
  PatchTx=tx:construct_tx(
              #{ver=>2,
                ...
                patches => [#{
                            ...
                            #{
                              <<"p">>=>[<<"current">>,<<"chainstate">>],
                              <<"t">>=><<"set">>,
                              <<"v">>=>Addr
                             },
                            #{
                              <<"p">>=>[<<"current">>,<<"autorun">>,<<"afterBlock">>],
                              <<"t">>=><<"set">>,
                              <<"v">>=>Addr
                             }
                           ]
  ...
  Patch=lists:foldl(fun(Key,Acc) ->
                                tx:sign(Acc, hex:decode(Key))
                            end, PatchTx, Privs),

  {ok, #{<<"txid">> := TxIDP}=ResPatch} = tpapi2:submit_tx(Node, Patch),
...
%
  ...
  {ok,TxId2} = make_transaction(Node,EAddr, My1Addr, Seq+3, <<"FTT">>,
                                 100000, <<"hello to EVM">>, EPriv, [nowait]),
  {ok,TxId3} = make_transaction(Node,EAddr, My1Addr, Seq+4, <<"SK">>,
                                 100, <<"SK 4 test">>, EPriv, [nowait]),
  ...
  BaseFile=filename:join(
             proplists:get_value("PWD",os:env(),"."),
             "examples/evm_builtin/build/LSTest"
            ),
  {ok,HexBin} = file:read_file(BaseFile++".bin"),
  ABI=contract_evm_abi:parse_abifile(BaseFile++".abi"),
  Code=hex:decode(hd(binary:split(HexBin,<<"\n">>))),
  ...
  {ok, #{<<"txid">> := _DTxID1, <<"block">>:=_Blkid1}=DeployRes} = tpapi2:submit_tx(Node, DeployTx),
  io:format("Deploy txid ~p~n~p~n",[_DTxID1, DeployRes]),
  ?assertMatch(#{<<"ok">> := true}, DeployRes),

  {ok, #{<<"block">> := _, <<"ok">> := true}} = tpapi2:wait_tx(Node, TxId3, erlang:system_time(second)+10),
  {ok, #{<<"block">> := _, <<"ok">> := true}} = tpapi2:wait_tx(Node, TxId2, erlang:system_time(second)+2),
  {ok, #{<<"block">> := _, <<"ok">> := true}} = tpapi2:wait_tx(Node, TxId1, erlang:system_time(second)+2),
  {ok, #{<<"block">> := _, <<"ok">> := true}} = tpapi2:wait_tx(Node, TxId0, erlang:system_time(second)+2),
  ...
  {ok, TxCall} = tpapi2:submit_tx(Node, CallTx1, [nowait]),
  io:format("TxIDCall: ~p~n",[TxCall]),
