...
%%    lager:debug("tpic peers: ~p", [Peers]),
    ...
    % Unconfirmed = [ #{ pub_key => 1 } ]
    Unconfirmed =
      lists:foldl(
        fun
          ({PeerPubKey, _, _}, Acc) ->
            maps:put(PeerPubKey, 1, Acc);
          (_, Acc) ->
            Acc
        end,
        #{},
...
send_batch(#{raw_batch := RawBatch} = State) ->
    tpic2:cast(<<"mkblock">>, {<<"txbatch">>, RawBatch}),
    wait_response(State),
    ok.
...
%% {'$gen_cast',{tpic,{61,4,<<5,102,134,118,0,0,5,193>>},<<"fake_tx_id">>}}
    {'$gen_cast', {tpic, From, BatchId}}  ->
      PubKey  = get_tpic_handle(From),
      Confirmed1 =
        case maps:is_key(PubKey , Unconfirmed) of
          false ->
      ...
      Unconfirmed1 = maps:remove(PubKey, Unconfirmed),
