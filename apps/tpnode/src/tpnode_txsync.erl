...
run_sync(TxID, TxBin, #{retry:=Retry}=Opts) ->
  ...
  lager:debug("Tx sync ~p Prepared ~p~n",[TxID, PList]),
...
sync_loop(TxID, _PushBin, [], Done, #{min_peers:=MP, notify:=Notify} = _Opts) ->
  if is_pid(Notify) ->
     ...
     true ->
       lager:notice("Bad notifier ~p",[Notify])
  end;
...
sync_loop(TxID, PushBin, PList, Done, #{min_peers:=MP}=Opts) ->
  ...
  receive
    {'$gen_cast',{tpic,{Peer, <<"txpool">>, _ReqID}, _Response}} ->
      lager:debug("Tx ~p Peer ~p done!~n",[TxID, Peer]),
...
cast_peers(TxID, PushBin, PL) ->
  lists:filtermap(
    fun({Peer, Retry}) ->
        if Retry==0 ->
             lager:notice("tx ~p peer ~p sync try count exceeded, abandon peer",
                         [TxID, Peer]),
             false;
           Retry>0 ->
