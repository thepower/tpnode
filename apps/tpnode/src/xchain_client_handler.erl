...
%%    ?LOG_INFO("Got pong for ~p", [_ConnPid]),
    Sub;
...
handle_xchain({outward_block, FromChain, ToChain, BinBlock}, _ConnPid, Sub) when
    is_integer(ToChain), is_integer(FromChain), is_binary(BinBlock) ->
    ?LOG_INFO("Got outward block from ~b to ~b", [FromChain, ToChain]),
    ...
    try
        ...
        ?LOG_ERROR("Can't dump inward block ~p:~p at ~p",
            [Ec, Ee, hd(S)])
    end,
    ?LOG_DEBUG("Here it is ~p", [Block]),
    gen_server:cast(txpool, {inbound_block, Block}),
    Sub;

handle_xchain({<<"subscribed">>,Cmd}, _ConnPid, Sub) ->
    ?LOG_INFO("xchain client: subscribed successfully ~s", [Cmd]),
    Sub;

handle_xchain({<<"unhandled">>,Cmd}, _ConnPid, Sub) ->
    ?LOG_INFO("xchain client: server did not understand my command: ~p", [Cmd]),
    Sub;

handle_xchain(Cmd, _ConnPid, Sub) ->
    ?LOG_INFO("xchain client got unhandled message from server: ~p", [Cmd]),
    Sub.
