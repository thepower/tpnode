...
contract ChainManagement is Ownable, ReentrancyGuard, Popcnt {
    ...
    uint256 public relock_timeout = 86400*7;  // 7 days
...
//    EnumerableSet.UintSet private availableNodeIds;
    ...
    function join(uint256 tokenId, bytes calldata pubkey) external nonReentrant {
        ...
        // Update lock renewal timestamp to 5 days from now
        lockRenewal[tokenId] = block.timestamp + 604800;

        emit NodeJoined(tokenId, pubkey);

        if(currentActive < max_nodes) {
          _cs_activate(tokenId);
        }
    }
    ...
    function activate(uint256 tokenId) public onlyOwner nonReentrant {
      require(inactiveNodes.contains(tokenId), "Token is not inactive");
    ...
    onlyOwner nonReentrant onlyActive(tokenId) returns(bool) {
      return _cs_deactivate(tokenId);
    }

    event update_debug(string,uint256,uint256);
    ...
    function epoch_update(
      ...
      emit update_debug("alive_count",alive_count,desired_consensus_nodes);
      ...
      if (alive_count < desired_consensus_nodes) {
        ...
        emit update_debug("eligible",nodes_to_add,selected_nodes_mask);
   ...
   */
    ...
    function _cs_activate(uint256 tokenId) internal returns (bool) {
      require(node_keys[tokenId].length > 0, "No pubkey found");
      bytes memory pubkey = node_keys[tokenId];

      (bool success, bytes memory data) = address(chainState).call(
        abi.encodeWithSignature("cm_register(bytes)", pubkey)
      );
      if(!success) return false;
      (uint8 nodeId) = abi.decode(data, (uint8));
    ...
    function _cs_deactivate(uint256 tokenId) internal returns (bool) {
        uint8 nodeId = activeNodeIdentifiers[tokenId];
        require(nodeId != 0, "Invalid node ID");
        (bool success, ) = address(chainState).call(
        abi.encodeWithSignature("cm_unregister(bytes)", nodeId)
      );
      if(!success) return false;
    ...
    function _cs_sync(uint256 tokenId) internal returns (bool) {
      require(node_keys[tokenId].length > 0, "No pubkey found");
      bytes memory pubkey = node_keys[tokenId];
      uint8 nodeId = activeNodeIdentifiers[tokenId];
      require(nodeId != 0, "Invalid node ID");
      (bool success, ) = address(chainState).call(
        abi.encodeWithSignature("cm_chkey(uint8, bytes)", nodeId, pubkey)
      );
      if(!success) return false;
      return true;
    }
    ...
    function _lock_test(uint256 tokenId) internal {
      chainNode.lock(tokenId, 7*86400);
    }

    function _lock(uint256 tokenId) internal {
      if(block.timestamp >= lockRenewal[tokenId]) {
        chainNode.lock(tokenId, lock_duration);
        lockRenewal[tokenId] = block.timestamp + relock_timeout;
      }
    }
    ...
    function _selectEligibleNodes(
      ...
      for (uint8 nodeId = 1; nodeId <= MAX_NODE_ID; nodeId++) {
        ...
        emit update_debug("sel",nodeId,tokenId);
